...
dataset_root = os.path.join(path, 'celeba_hq_256')  # Use the downloaded dataset path
...
# DataLoader optimization for better performance on CUDA.
kwargs = {'num_workers': 8, 'pin_memory': True}

# Create DataLoaders for train and test datasets
train_dataloader = DataLoader(train_dataset, batch_size=32, shuffle=True, **kwargs)
test_dataloader = DataLoader(test_dataset, batch_size=32, shuffle=False, **kwargs)

def show_images(hr_images, lr_images, n=5):
    """
    Display a side-by-side comparison of high-resolution (HR) and low-resolution (LR) images.

    Args:
        hr_images (torch.Tensor): Batch of high-resolution images in [-1, 1] range.
        lr_images (torch.Tensor): Batch of low-resolution images in [-1, 1] range.
        n (int, optional): Number of image pairs to display. Default is 5.

    Displays:
        A matplotlib figure with `n` rows and 2 columns, where each row contains
        a pair of HR and LR images. Images are denormalized to [0, 1] for visualization.
    """

    def denormalize(img):
        return (img * 0.5) + 0.5

    fig, axes = plt.subplots(nrows=n, ncols=2, figsize=(4, 2 * n), dpi=150)

    for i in range(n):
        hr_img = denormalize(hr_images[i]).cpu().numpy().transpose(1, 2, 0)
        lr_img = denormalize(lr_images[i]).cpu().numpy().transpose(1, 2, 0)
...
for hr_real, lr_real in train_dataloader:
    show_images(hr_real, lr_real, n=5)
    break
...
class Loss(nn.Module):
    ...
    def forward(self, generator, discriminator, hr_real, lr_real):
        ...
        g_loss = (
            0.002 * self.adv_loss(fake_preds_for_g, True) +
            0.012 * self.vgg_loss(hr_real, hr_fake) +
...
def train_srresnet(generator, dataloader, device, lr=1e-4, total_steps=30_000, display_step=1000, patience=5, min_improvement=0.1, warmup_steps=10_000):
    ...
    plot_metrics(psnr_values, title="PSNR During SRResNet Training", ylabel="PSNR (dB)")
    plot_metrics(ssim_values, title="SSIM During SRResNet Training", ylabel="SSIM")
    plot_metrics(loss_values, title="Loss During SRResNet Training", ylabel="MSE Loss")
...
def train_srgan(generator, discriminator, dataloader, device, lr=1e-4, total_steps=30_000, display_step=1000, patience=5, min_improvement=0.1, warmup_steps=10000):
    ...
    # Plot metrics after training
    plot_metrics(psnr_values, title="PSNR During SRGAN Training", ylabel="PSNR (dB)")
    plot_metrics(ssim_values, title="SSIM During SRGAN Training", ylabel="SSIM")
    plot_metrics(g_loss_values, title="Generator Loss During SRGAN Training", ylabel="Generator Loss")
    plot_metrics(d_loss_values, title="Discriminator Loss During SRGAN Training", ylabel="Discriminator Loss")
...
discriminator = Discriminator(n_blocks=3, base_channels=64)
